...
public class PotionBrewing {
   ...
   public static final List<PotionBrewing.Mix<Item>> CONTAINER_MIXES = Lists.newArrayList();
   ...
   protected static boolean hasContainerMix(ItemStack pInput, ItemStack pReagent) {
      ...
      for(int j = CONTAINER_MIXES.size(); i < j; ++i) {
         ...
         if (mix.from.get() == item && mix.ingredient.test(pReagent)) {
            return true;
         }
      }
   ...
   public static class Mix<T> {
      ...
      public Mix(net.minecraftforge.registries.IForgeRegistry<T> registry, T pFrom, Ingredient pIngredient, T pTo) {
         this.from = registry.getDelegateOrThrow(pFrom);
         this.ingredient = pIngredient;
         this.to = registry.getDelegateOrThrow(pTo);
      }
   }
}
